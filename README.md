...
## Setup
Note that the commands that follow are run on my Arch Linux system with nvim text editor but they can similarly be ran on other OSes and systems. Reach out for help specific to your system.
...
3. Using the same virtual environment includes all the packages you need, in which case you can ignore this step, but I have included a requirements.txt that contains the essential packages regardless in case you want to use some other env. You can download them by running the following in your env:
```
...
Make sure the path to the PDF is correct. Requirement for setting up the library differs from OS to OS, so make sure to refer to the documentation cited below [1].

## Outline the PDF
Some manual work has to be done due to the arbitrariness in the format of the content pages of books.

1. Get the starting and ending PDF page numbers of the content pages: `content_start_page` and `content_end_page`.
...
4. To add custom bookmarks, follow the same format and add anything (as shown in the last line of the example) with the correct PDF page number.
